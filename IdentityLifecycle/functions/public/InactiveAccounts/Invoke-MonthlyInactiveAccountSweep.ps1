function Invoke-MonthlyInactiveAccountSweep {
    <#
    .SYNOPSIS
        Stateless monthly sweep that evaluates pre-identified inactive privileged accounts
        against absolute inactivity thresholds and takes the required action in a single pass.

    .DESCRIPTION
        Designed for a once-monthly run where notification fatigue is not a concern.
        Unlike Invoke-InactiveAccountSweep (which maintains stage state in Azure Table
        Storage across daily runs), this function is entirely stateless: given a list of
        accounts (e.g. from a dashboard export), it computes how long each account has
        been inactive and takes the highest-severity action that applies.
        ...
        For all accounts, owner is resolved in this order:
            ...
            2. extensionAttribute14: parse 'owner=<sam>' from semicolon-delimited key=value
               pairs (fallback for accounts that don't follow the naming convention).
               Only available for AD-routed accounts where extensionAttribute14 was fetched
               from the live query; Entra-native accounts have no EA14.
    ...
    try {
        ...
        Write-Warning "Invoke-MonthlyInactiveAccountSweep failed: $($output.Error)"
    }
    finally {
        ...
        Write-Host ("Monthly sweep complete: {0} warned, {1} disabled, {2} deleted, " +
