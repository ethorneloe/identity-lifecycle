<#
.SYNOPSIS
    Test harness for Invoke-MonthlyInactiveAccountSweep.

.DESCRIPTION
    Runs all scenarios defined in the scenarios/ subdirectory against the real monthly
    orchestrator, with AD, Entra, Graph-mail, and action dependencies replaced by
    in-memory mocks injected into the module's script scope.
    ...
    Architecture:
        ...
        - UseExistingGraphSession and SkipModuleImport are both passed on every call so
          tests run without real AD/Graph infrastructure or module imports.
        - Get-ADUser, Get-MgUser, Send-GraphMail, Disable-InactiveAccount, and
          Remove-InactiveAccount are mocked inside the module scope.
        - Resolve-EntraSignIn, ConvertTo-Bool, Get-ADAccountOwner, and
          New-InactiveAccountLifecycleMessage run for real. Get-ADAccountOwner resolves via the
          mocked Get-ADUser, so no separate mock is needed.
        - Each scenario supplies -Accounts (input rows), -ADUsers (mock AD data),
          and optionally -MgUsers, -NotifyFail, -DisableFail, -RemoveFail.
...
. (Join-Path $TestRoot 'helpers\New-MonthlyTestAccount.ps1')
. (Join-Path $TestRoot 'helpers\Assert-MonthlyResult.ps1')
. (Join-Path $TestRoot 'helpers\Set-MonthlyMocks.ps1')

Set-MonthlyMocks -MockContext $script:MockContext

# ---------------------------------------------------------------------------
# RUNNER HELPERS
# ---------------------------------------------------------------------------

function Invoke-MonthlyOnce {
    param(
        [object[]] $Accounts,
    ...
    Invoke-MonthlyInactiveAccountSweep @params
}
...
function Invoke-Scenario {
    ...
    $accounts = @(if ($Scenario.ContainsKey('Accounts')) { $Scenario.Accounts } else { @() })
    ...
    $result = Invoke-MonthlyOnce @invokeParams
...
Write-Host ''
Write-Host 'Identity Lifecycle Monthly Sweep - Test Harness' -ForegroundColor White
...
$reportPath   = Join-Path $ReportsDir ("MonthlyTest-" + $genTimestamp + ".html")
...
$lines.Add('<title>Identity Lifecycle Monthly Sweep - Test Report</title>')
$lines.Add('<style>')
...
$lines.Add('</head>')
$lines.Add('<body>')
$lines.Add('<h1>Identity Lifecycle Monthly Sweep - Test Report</h1>')
