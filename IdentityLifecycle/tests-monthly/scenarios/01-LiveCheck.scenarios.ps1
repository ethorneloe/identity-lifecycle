$Scenarios = @(
    ...
    $(
        ...
        @{
            ...
            Accounts = @(
                New-MonthlyTestAccount -SamAccountName $sam -UPN $upn -InactiveDaysAgo 30
            )
            ADUsers = @{
...
"@)
    ...
    $(
        ...
        @{
            ...
            ADUsers = @{}
            MgUsers = @{
                $oid.ToLower() = New-MonthlyMgUser -ObjectId $oid -AccountEnabled $false -LastSignInDaysAgo 150
            }
            AssertAfterRun = [scriptblock]::Create(@"
                param(`$result, `$ctx)
                Assert-SummaryField `$result.Summary 'Skipped' 1 'Skipped = 1'
