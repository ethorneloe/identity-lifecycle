<#
.SYNOPSIS
    Test harness for Invoke-AccountInactivityRemediationWithImport.

.DESCRIPTION
    Runs all scenarios defined in the scenarios/ subdirectory against the real WithImport
...
function Invoke-WithImportOnce {
    param(
        [object[]] $Accounts,
...
Write-Host ''
Write-Host 'Invoke-AccountInactivityRemediationWithImport - Test Harness' -ForegroundColor White
...
$reportPath   = Join-Path $ReportsDir ("WithImportTest-" + $genTimestamp + ".html")
...
$lines.Add('<title>Invoke-AccountInactivityRemediationWithImport - Test Report</title>')
$lines.Add('<style>')
...
$lines.Add('</head>')
$lines.Add('<body>')
$lines.Add('<h1>Invoke-AccountInactivityRemediationWithImport - Test Report</h1>')
