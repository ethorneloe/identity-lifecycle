$Scenarios = @(
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                New-RemediationADAccount -SamAccountName $sam -UPN $upn -LastLogonDaysAgo 30 -WhenCreatedDaysAgo 200
            )
            ADUsers  = @{
...
"@)
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                New-RemediationADAccount -SamAccountName $sam -UPN $upn -LastLogonDaysAgo 120 -WhenCreatedDaysAgo 300 -Enabled $false
            )
            ADUsers  = @{
                $std = New-RemediationOwnerADUser -SamAccountName $std -EmailAddress "$std@corp.local"
            }
...
"@)
    ...
    $(
        ...
        @{
            ...
            EnableDeletion    = $true
            ADAccountList     = @(
                New-RemediationADAccount -SamAccountName $sam -UPN $upn -LastLogonDaysAgo 180 -WhenCreatedDaysAgo 500 -Enabled $false
            )
            ADUsers           = @{
                $std = New-RemediationOwnerADUser -SamAccountName $std -EmailAddress "$std@corp.local"
            }
...
"@)
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                New-RemediationADAccount -SamAccountName $samAD -UPN $upnAD -LastLogonDaysAgo 95 -WhenCreatedDaysAgo 300
            )
            EntraAccountList = @(
                New-RemediationEntraAccount -EntraObjectId $oidEN -UPN $upnEN -LastSignInDaysAgo 95
            )
            ADUsers  = @{
                $stdAD = New-RemediationOwnerADUser -SamAccountName $stdAD -EmailAddress "$stdAD@corp.local"
            }
            AssertAfterRun = [scriptblock]::Create(@"
                param(`$result, `$ctx)
