$Scenarios = @(
    ...
    $(
        ...
        @{
            ...
            EntraAccountList = @(
                New-DirectEntraAccount -EntraObjectId $oid -UPN $upn -LastSignInDaysAgo 5
            )
...
"@)
    ...
    $(
        ...
        @{
            ...
            EntraAccountList = @(
                New-DirectEntraAccount -EntraObjectId $oid -UPN $upn -LastSignInDaysAgo 95
            )
            AssertAfterRun = [scriptblock]::Create(@"
                param(`$result, `$ctx)
                Assert-SummaryField `$result.Summary 'Skipped' 1 'Skipped = 1'
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                ...
                New-DirectADAccount -SamAccountName $sam -UPN $upn -LastLogonDaysAgo 120 -WhenCreatedDaysAgo 400
            )
            EntraAccountList = @(
                # Synced account -- same UPN; sign-in 85 days ago
                New-DirectEntraAccount -EntraObjectId $oid -UPN $upn `
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                ...
                New-DirectADAccount -SamAccountName $sam -UPN $upn -LastLogonDaysAgo 85 -WhenCreatedDaysAgo 400
            )
            EntraAccountList = @(
                New-DirectEntraAccount -EntraObjectId $oid -UPN $upn `
...
"@)
    ...
    $(
        ...
        @{
            ...
            ADAccountList = @(
                New-DirectADAccount -SamAccountName $samA -UPN $upnA -LastLogonDaysAgo 120 -WhenCreatedDaysAgo 300
                # mix05b strips to 'mix05b' -- not in ADUsers, no owner
                New-DirectADAccount -SamAccountName $samB -UPN $upnB -LastLogonDaysAgo 120 -WhenCreatedDaysAgo 300
            )
            EntraAccountList = @(
                New-DirectEntraAccount -EntraObjectId $oidC -UPN $upnC -LastSignInDaysAgo 95
            )
            ADUsers  = @{
                $stdA = New-DirectOwnerADUser -SamAccountName $stdA -EmailAddress "$stdA@corp.local"
            }
            AssertAfterRun = [scriptblock]::Create(@"
                param(`$result, `$ctx)
