<#
.SYNOPSIS
    Test harness for Invoke-DirectInactiveAccountSweep.

.DESCRIPTION
    Runs all scenarios defined in the scenarios/ subdirectory against the real direct
...
. (Join-Path $TestRoot 'helpers\New-DirectTestAccount.ps1')
. (Join-Path $TestRoot 'helpers\Assert-DirectResult.ps1')
. (Join-Path $TestRoot 'helpers\Set-DirectMocks.ps1')

Set-DirectMocks -MockContext $script:MockContext

# ---------------------------------------------------------------------------
# RUNNER HELPERS
# ---------------------------------------------------------------------------

function Invoke-DirectOnce {
    param(
    ...
    Invoke-DirectInactiveAccountSweep @params
}
...
function Invoke-Scenario {
    ...
    $result = Invoke-DirectOnce @invokeParams
...
Write-Host ''
Write-Host 'Identity Lifecycle Direct Sweep - Test Harness' -ForegroundColor White
...
$reportPath   = Join-Path $ReportsDir ("DirectTest-" + $genTimestamp + ".html")
...
$lines.Add('<title>Identity Lifecycle Direct Sweep - Test Report</title>')
$lines.Add('<style>')
...
$lines.Add('</head>')
$lines.Add('<body>')
$lines.Add('<h1>Identity Lifecycle Direct Sweep - Test Report</h1>')
